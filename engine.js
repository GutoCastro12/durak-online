...
function createGame(numPlayers, startIdx=0, rng=Math.random, attackLimit=4){
  ...
  const G = {
    ...
    attackLimit: (attackLimit && attackLimit>0) ? attackLimit : Infinity,  // Infinity = "sem limite" (limita só pela mão do defensor)
    attackerIdx:startIdx, defenderIdx:nextActive(players,startIdx,1),
    table:[], phase:'attack',
    passers:new Set(),                 // attacker ids who passed this round
    discardCount:0, finished:false, durak:null, log:[], _logSeq:0, finishOrder:[],
...
function L(G,msg,big=false){ G._logSeq=(G._logSeq||0)+1; G.log.unshift({msg,big,seq:G._logSeq}); }
...
function canAddMore(G){
  ...
  if(G.table.length>=G.attackLimit) return false;
  ...
  return G.table.length<G.attackLimit;
}
...
function legalAttackCap(G){
  // max cards on the table = min(attackLimit, defenderHand + alreadyDefended).
  // "Sem limite" (attackLimit=Infinity) collapses to just the defender's hand capacity.
  return Math.min(G.attackLimit, G.players[G.defenderIdx].hand.length + countDefended(G));
}
