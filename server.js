...
function createRoom(hostConnId, hostName, numSeats, attackLimit) {
  ...
  const room = { code, hostId, started:false, numSeats, attackLimit: (attackLimit===0?0:(attackLimit||4)), seats, G:null, conns:new Map(), botTimer:null, coTimer:null, emptyTimer:null };
  rooms.set(code, room);
  return { room, hostId };
}
...
function isWaitingOnConnectedHuman(room){
  ...
  // co-attack window: wait only for a connected human who CAN actually add a card
  if (G.phase==='attack' && G.table.length>0 && E.undefended(G).length===0){
    ...
    for (const i of E.attackerOrder(G)){
      ...
      if (seatActsAsBot(room, i)) continue;
      const canAdd = G.table.length < cap && G.players[i].hand.some(c=>ranks.has(c.rank));
      if (canAdd) return true;            // a human with a real option -> give them the grace window
    }
    return false;
  }
...
// Auto-pass any attacker (human OR bot) who literally cannot add a card right now,
// so nobody is ever forced to press "Passar" when they have no legal move.
function autoPassStuckCoAttackers(room){
  ...
  for (const i of E.attackerOrder(G)){
    ...
    if (G.passers.has(i)) continue;
    const canAdd = G.table.length < cap && G.players[i].hand.some(c=>ranks.has(c.rank));
    if (!canAdd){ E.doPass(G, i); if (G.finished) return; }   // human or bot — no choice, so pass
  }
}
...
function pump(room){
  ...
  // --- Co-attack window: simultaneous, time-boxed. Anyone who CAN add gets a short
  //     window to jump in; nobody is forced to pass. Bots add/decline quickly. ---
  if (G.phase==='attack' && G.table.length>0 && E.undefended(G).length===0){
    autoPassStuckCoAttackers(room);                 // anyone (human/bot) with no legal add passes
    if (allCoAttackersDone(room)){                  // everyone done -> bout won
      clearCoWindow(room);
      E.resolveBoutWon(G);
    ...
    // bots that can add act first (fast), so a quick human can still beat them
    ...
    // only humans-who-can-add remain. Give them a grace window, then auto-resolve.
    if (isWaitingOnConnectedHuman(room)){
      broadcast(room);
      startCoWindow(room);                          // arms a one-shot timer if not already armed
      return;
    }
    // no one left to act
    clearCoWindow(room);
  ...
  clearCoWindow(room);   // left the co-attack window
...
// --- co-attack grace window ---
// When the only ones left who could pile on are humans, give them a few seconds.
// If they don't act, treat them as passing and resolve. This makes co-attacking an
// opportunity, never an obligation.
const CO_WINDOW_MS = 4500;
function startCoWindow(room){
  if (room.coTimer) return;                  // already armed
  room.coTimer = setTimeout(()=>{
    room.coTimer = null;
    const G = room.G;
    if (!G || G.finished) return;
    // still in the co-attack window? auto-pass remaining human co-attackers.
    if (G.phase==='attack' && G.table.length>0 && E.undefended(G).length===0){
      for (const i of E.attackerOrder(G)){
        if (i===G.defenderIdx) continue;
        if (!G.passers.has(i)) E.doPass(G, i);
        if (G.finished) break;
      }
      broadcast(room);
      if (!G.finished) { syncControl(room); pump(room); }
    }
  }, (process.env.FAST ? 300 : CO_WINDOW_MS));
}
function clearCoWindow(room){
  if (room.coTimer){ clearTimeout(room.coTimer); room.coTimer = null; }
}
...
function legalFor(room, seatIdx){
  ...
  if (G.phase==='attack'){
    ...
    // CO-ATTACK window (all cards defended): you only get a choice if you CAN add a card.
    // If you can't add anything, the server auto-passes you — no forced "Passar".
    if (E.undefended(G).length===0){
      ...
      if (G.table.length < cap){
        ...
        if (addable.length){
          out.youract = true;
          out.canAttack = true;
          out.attackRanks = [...new Set(addable)];
          out.canPass = true;                             // can also choose to opt out now
        }
      }
      return out;                                         // nothing addable -> youract stays false
    }
  }
  return out;
}
...
function applyAction(room, seatIdx, msg){
  const G = room.G;
  if (!G || G.finished) return;
  clearCoWindow(room);          // a human acted; cancel any pending grace-window timer
  syncControl(room);
  const p = G.players[seatIdx];
  if (!p || p.out) return;
  ...
  if (msg.action==='attack'){
    ...
    if (G.table.length===0){
      // opening attack: all cards must share the same rank
      if (new Set(cards.map(c=>c.rank)).size>1) return;
      if (G.attackerIdx!==seatIdx) return;
      if (cards.length > E.legalAttackCap(G)) return;
      E.doAttack(G, seatIdx, cards);
    } else {
      // co-attack: each card's rank must already be on the table (ranks may differ from each other)
...
wss.on('connection', (ws)=>{
  ...
  ws.on('message', (raw)=>{
    ...
    if (msg.type==='join'){
      ...
      if (room.started){
        ...
        if (seat>=0){
          clearTimeout(room.emptyTimer); room.emptyTimer=null;   // someone came back; don't discard the room
    ...
    if (msg.type==='leave'){
      ...
      if (seat>=0){
        if (!room.started && seat===0){
          ...
          clearTimeout(room.botTimer); clearCoWindow(room); rooms.delete(room.code);
          return;
        }
        if (room.started){
      ...
      if (room.conns.size===0){ clearTimeout(room.botTimer); clearCoWindow(room); rooms.delete(room.code); return; }
      broadcast(room);
  ...
  ws.on('close', ()=>{
    ...
    if (room.conns.size===0){
      clearTimeout(room.botTimer); room.botTimer=null; clearCoWindow(room);
      if (!room.started){
        rooms.delete(room.code);               // lobby with nobody -> drop immediately
      } else {
        // in-progress game emptied (often a transient network blip / server wake).
        // Keep it alive briefly so players can reconnect to the same room & seats.
        clearTimeout(room.emptyTimer);
        room.emptyTimer = setTimeout(()=>{
          const r = rooms.get(room.code);
          if (r && r.conns.size===0){ clearTimeout(r.botTimer); clearCoWindow(r); rooms.delete(r.code); }
        }, 90000);                             // 90s grace before discarding the game
      }
    }
  });
});
