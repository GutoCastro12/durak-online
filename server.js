...
function createRoom(hostConnId, hostName, numSeats, attackLimit) {
  ...
  const room = { code, hostId, started:false, numSeats, attackLimit: (attackLimit===0?0:(attackLimit||4)), seats, G:null, conns:new Map(), botTimer:null, coTimer:null };
  rooms.set(code, room);
  return { room, hostId };
}
...
wss.on('connection', (ws)=>{
  ...
  ws.on('close', ()=>{
    ...
    // cleanup empty rooms
    if (room.conns.size===0){ clearTimeout(room.botTimer); clearCoWindow(room); rooms.delete(room.code); }
  });
});
